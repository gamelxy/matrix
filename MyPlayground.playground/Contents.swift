...
internal typealias Position = (Int, Int)
...
class Matrix: CustomDebugStringConvertible {
  ...
  func contain(matrix: Matrix) -> Bool {
    guard
      let (positions, count) = find(matrix: matrix),
      positions.count == count else {
        return false
    }
    return count > 0
  }
  
  func find(matrix: Matrix) -> (positions: [Position], count: Int)? {
    guard
    ...
    for index in 0...(self.rows - matrix.rows) {
      ...
      for subIndex in 1..<matrix.rows {
        ...
        if (vectorEqual(vector: subVector, subVector: matrix.data[subIndex])) {
          searchLength -= 1
          if (searchLength == 0) {
            positions.append(Position(index, vectorIndex))
            searchLength = matrix.rows
          }
        }
      }
    }
...
class MatrixTest {
  ...
  class func testMath() {
    ...
    let matrixB2 = Matrix(data: [[27, 12],
                                 [14, 13]])
    let matrixB3 = Matrix(data: [[32, 13],
                                 [21, 32]])
    ...
    if let (positions, count) = matrixA.find(matrix: matrixA2) {
      print("MatrixA posions A2: \(positions), count: \(count)")
    }
    if let (positions, count) = matrixB.find(matrix: matrixB1) {
      print("MatrixB posions B1: \(positions), count: \(count)")
    }
    if let (positions, count) = matrixB.find(matrix: matrixB2) {
      print("MatrixB posions B2: \(positions), count: \(count)")
    }
    if let (positions, count) = matrixB.find(matrix: matrixB3) {
      print("MatrixB posions B3: \(positions), count: \(count)")
    }
  }
}
